...
FORWARD _PROTOTYPE( int hp_hw_int, (int irq) );
...
/* A keyboard interrupt has occurred.  Process it. */
  ...
  code = scan_keyboard();
  if (hp200)
  	hp_timeout = proc_addr(IDLE)->user_time + hp_tm_reset;
...
/* This procedure traps function keys for debugging and control purposes. */
  ...
  if (code < F1 || code > ON) return(FALSE);	/* not our job */

  switch (map_key(scode)) {			/* include modifiers */

  case ON:	hp_off(); break;	/* turn an HP palmtop off */
